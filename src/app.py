...
@login_required
def leaderboard():
    # Get all scores from the database, sorted primarily by score and secondarily by timestamp
    scores = Score.query.order_by(Score.score.desc(), Score.timestamp.desc()).all()
    ...
    # Note: Lists are already sorted by game because the main query was sorted
